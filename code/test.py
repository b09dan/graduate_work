...
for url_article in final_links:  # Through loop searching for articles' text
    try:
        ...
        temp_published_time = BeautifulSoup(str(source_article.find_all('time', class_="date", limit=1)),
                                            "html.parser").text

        temp_theme = BeautifulSoup(str(source_article.find_all('span', class_="tags", limit=1)),
                                            "html.parser").text
